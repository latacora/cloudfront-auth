...
//   }
// }
// Client ID from g cloud oauth client
// Client Secret from g cloud oauth client
// redirectURI during the subdomain generated in the account. Should be /_callback
...
function googleConfiguration(clientId, clientSecret, redirectURI, hostedDomain, sessionDuration, authz, temp_dir) {
    ...
    config.CALLBACK_PATH = url.parse(redirectURI).pathname;
    config.HOSTED_DOMAIN = hostedDomain;

    config.AUTH_REQUEST.client_id = clientId;
    config.AUTH_REQUEST.response_type = 'code';
    config.AUTH_REQUEST.scope = 'openid email';
    config.AUTH_REQUEST.redirect_uri = redirectURI;
    config.AUTH_REQUEST.hd = hostedDomain;

    config.TOKEN_REQUEST.client_id = clientId;
    config.TOKEN_REQUEST.client_secret = clientSecret;
    config.TOKEN_REQUEST.redirect_uri = redirectURI;
    config.TOKEN_REQUEST.grant_type = 'authorization_code';
